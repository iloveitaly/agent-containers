...
- [`cursor/install.sh`](cursor/install.sh) installs Docker/mise/direnv, global mise `gh`/`uv`, and `iloveitaly/gh-ai-pr` via passwordless sudo. [`cursor/start.sh`](cursor/start.sh) starts dockerd each boot.
