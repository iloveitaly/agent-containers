...
- [`cursor/install.sh`](cursor/install.sh) installs Docker/mise/direnv, global mise `uv`, `gh` via mise only if missing, and `iloveitaly/gh-ai-pr` via passwordless sudo. [`cursor/start.sh`](cursor/start.sh) starts dockerd each boot.
