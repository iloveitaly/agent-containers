...
- [`cursor/install.sh`](cursor/install.sh) installs Docker/mise/direnv via passwordless sudo. If a justfile exists at `/workspace` or `$PWD` and defines `setup`, it runs `just setup` as `ubuntu`. [`cursor/start.sh`](cursor/start.sh) starts dockerd each boot.
