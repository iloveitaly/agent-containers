...
- **mise** — language/tool version management with shell activation by default; `/workspace` pre-trusted; [`gh`](https://cli.github.com/) and [`uv`](https://docs.astral.sh/uv/) are global mise tools
...
cursor/             # Cursor cloud-agent style image
  ...
  install.sh        # Docker, zsh, ubuntu user, mise (+ global gh/uv), direnv, gh-ai-pr (+ /workspace trust)
...
[`cursor/install.sh`](cursor/install.sh) re-execs with passwordless `sudo` so it can write Docker's apt key under `/etc/apt/keyrings` (otherwise `gpg --dearmor` fails with `Permission denied`). It also installs **zsh**, **mise**, and **direnv** when missing, installs **gh** and **uv** as global mise tools (`~/.config/mise/config.toml`), installs the [`iloveitaly/gh-ai-pr`](https://github.com/iloveitaly/gh-ai-pr) extension as `ubuntu`, and trusts `/workspace`.
