...
- **mise** — language/tool version management with shell activation by default; `/workspace` pre-trusted; [`uv`](https://docs.astral.sh/uv/) is a global mise tool
- **direnv** — per-directory env loading, hooked *after* mise so PATH stays consistent; `/workspace` whitelisted
- **GitHub CLI** — `gh` plus the [`iloveitaly/gh-ai-pr`](https://github.com/iloveitaly/gh-ai-pr) extension for the `ubuntu` user
...
cursor/             # Cursor cloud-agent style image
  ...
  install.sh        # Docker, zsh, ubuntu user, mise (+ global uv), direnv, gh, gh-ai-pr (+ /workspace trust)
...
[`cursor/install.sh`](cursor/install.sh) re-execs with passwordless `sudo` so it can write Docker's apt key under `/etc/apt/keyrings` (otherwise `gpg --dearmor` fails with `Permission denied`). It also installs **zsh**, **mise**, and **direnv** when missing, installs **uv** as a global mise tool (`uv = "latest"` in `~/.config/mise/config.toml`), installs **GitHub CLI** (`gh`) when missing, installs the [`iloveitaly/gh-ai-pr`](https://github.com/iloveitaly/gh-ai-pr) extension as `ubuntu`, and trusts `/workspace`.
